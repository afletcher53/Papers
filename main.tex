...
\newacronym{sal}{SAL}{Simple Active Learning}
...
\newacronym{fpt}{FPT}{Further Pretraining}
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % Other commands
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
linkcolor=blue,
citecolor=blue,
pdftitle={Efficient Screening in Medical Systematic Reviews - Aaron HA Fletcher},
pdfsubject={Systematic Review},
pdfauthor={Aaron HA Fletcher},
pdfkeywords={Systematic Reveiws; Technology-Assisted Review; Citation Analysis; Metadata; Stopping Methods}
}
...
\end{figure}
\footnotetext{Reproduced from \cite{ghasemi_scientific_2022} under the CC BY-NC-ND 4.0 license \url{https://creativecommons.org/licenses/by-nc-nd/4.0/}. No changes were made. Noncommercial use only.}

%The result = Out-of-date information at publication
...
%More Systematic Reviews are being created
...
\end{figure}
\footnotetext{Reproduced from \cite{tawfik_protocol_2020} under the CC BY 4.0 license \url{https://creativecommons.org/licenses/by/4.0/}. No changes made.}
...
\begin{figure}[h]
    ...
    \end{tikzpicture}
    \caption{Node feature enrichment process using an \gls*{llm}. A \gls*{llm} is prompted to generate features from a document, a process called feature enrichment.}
    \label{fig:llm4g}
    \end{figure}
...
\begin{table}[htbp]
    ...
    \end{tabular}
    \caption{Performance comparison across different collections and models. While some improvements in effectiveness (increased R-Precision) were seen, this was not consistent across all datasets.}
    \label{tab:results}
\end{table}
...
\caption{Document ranking and selection before and after an Active Learning iteration. Note that the selection policy used here is relevance with a batch size of three (i.e., the highest three ranked documents are selected for oracle judgement.}
...
% \begin{figure}
%     \centering
%     \includesvg[width=1\linewidth]{Confirmation Review/images/simulated_run.svg}
%     \caption{Demonstration of effect of score adjustment on rankings. A negative rank difference means the document moves closer to the top of the rankings, resulting in a higher chance of selection by a relevant policy. Increasing $alpha$ increases the magnitude of this adjustment.}
%     \label{fig:simulated_adjusted}
% \end{figure}
...
\end{tikzpicture}
\caption{Specific task details for Research question 3.}
...
\includepdf[pages=-]{Confirmation Review/supporting_work/COM61003.pdf}
\includepdf[pages=-]{Confirmation Review/supporting_work/COM61004.pdf}
